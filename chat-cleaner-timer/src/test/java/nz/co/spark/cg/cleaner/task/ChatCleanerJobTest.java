...
public class ChatCleanerJobTest {
    ...
    public void checkTimerConfig() {
        ...
        assertEquals(chatDBCleanerTimer.getJobClass(),ChatCleanerJob.class);
        assertEquals(chatDBCleanerTimer.isConfigEnabled(),true);
    }

    @Test
    public void checkCleanerTimerAttributes() {
        assertFalse(chatDBCleanerTimer.cronExpression().isEmpty());
        assertEquals(chatDBCleanerTimer.getTriggerType(),TriggerType.CRON_EXPRESSION_TRIGGER);
        assertEquals(chatDBCleanerTimer.getJobClass(),ChatCleanerJob.class);
        assertEquals(chatDBCleanerTimer.getName(),"chat-cleaner-timer");
        assertFalse(chatDBCleanerTimer.getDescription().isEmpty());
        assertTrue(chatDBCleanerTimer.isConfigEnabled());
        assertEquals(chatDBCleanerTimer.getType(), DriverType.Custom);
    }

    @Test
    public void checkCleanerJob() throws SQLException {
        ChatCleanerJob chatCleanerJob = new ChatCleanerJob();
        String sql = "SELECT COUNT(*) FROM interaction WHERE type='Chat';";
        Statement statement = memConnection.createStatement();
        int record = -1;

        createMockData();
        chatCleanerJob.execute(null);
    ...
    private void createMockData() throws SQLException {
        ...
        final LocalDateTime yesterday = LocalDateTime.now().minusDays(2);
