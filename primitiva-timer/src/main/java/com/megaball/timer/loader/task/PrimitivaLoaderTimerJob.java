...
public class PrimitivaLoaderTimerJob implements Job {
    ...
    public PrimitivaLoaderTimerJob() {
    }
    ...
    private void writeNewRecordToDB(final Connection connection, final Map<String,String> values, long[] yearCounter) throws SQLException {
        ...
        findByGameIdAndDate.setDate(2,Date.valueOf(LocalDate.parse(values.get("date"), DateTimeFormatter.ofPattern("d/MM/yyyy"))));
        ...
        if (rs.next()) {
            ...
            insertStmt.setDate(2,Date.valueOf(LocalDate.parse(values.get("date"), DateTimeFormatter.ofPattern("d/MM/yyyy"))));
